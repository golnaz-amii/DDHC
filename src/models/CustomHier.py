...
class CustomHier(BaseModel):
    def __init__(
        ...
        self.level_4_model = XGBClassifier(random_state=self.seed)
    ...
    def transform_datasset(self, X_train, y_train, X_eval, y_eval, X_test, y_test):
        ...
        # classifier level 3: small vs large vs rest
        ...
        y_train_level_3 = y_train[mask_level_3_train].apply(
            lambda x: 2 if x == 5 else (1 if x == 4 else 0)
        )
        X_test_level_3 = X_test[mask_level_3_test]
        y_test_level_3 = y_test[mask_level_3_test].apply(
            lambda x: 2 if x == 5 else (1 if x == 4 else 0)
        )
        ...
        # classifier level 4: transitional vs intermittent
        mask_level_4_train = (y_train != 0) & (y_train != 1) & (y_train != 4) & (y_train != 5)
        mask_level_4_test = (y_test != 0) & (y_test != 1) & (y_test != 4) & (y_test != 5)
