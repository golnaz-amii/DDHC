...
class HierDomain(BaseModel):
    '''
    Level 1: (no and ephemeral) vs (intermittent and transitional) vs (small vs large)
    Level 2:
    Classifier 1: no vs ephemeral
    Classifier 2: intermittent vs transitional
    Classifier 3: small vs large
...
+-------+-------+ +-------+-------+ +-------+-------+
|               | |               | |               |
| Level 2 Model | | Level 2 Model | | Level 2 Model |
|      1        | |      2        | |      3        |
|               | |               | |               |
+-------+-------+ +-------+-------+ +-------+-------+
        |                 |                 |
        v                 v                 v
      no vs            intermittent       small vs
    ephemeral        vs transitional       large
    '''
    def __init__(
        self,
        dict_labels,
    ...
    def transform_datasset(self, X_train, y_train, X_eval, y_eval, X_test, y_test):
        ...
        # classifier level 1: (no and ephemeral) vs (intermittent and transitional) vs (small and large)
        ...
        # classifier level 2.1: no vs ephemeral
        ...
        # classifier level 2.2: intermittent vs transitional
    ...
    def predict_proba(self, X_test):
        ...
        for i in range(n_samples):
            p_no_ephemeral = proba_level_1[
                i, 0
            ]  # Probability of class 0 (No or Ephemeral)
            p_inter_trans = proba_level_1[
                i, 1
            ]  # Probability of class 1 (Intermittent or Transitional)
            p_small_large = proba_level_1[
                i, 2
            ]  # Probability of class 2 (Small or Large)

            p_no = proba_level_2_1[i, 0] * p_no_ephemeral
            p_ephemeral = proba_level_2_1[i, 1] * p_no_ephemeral
